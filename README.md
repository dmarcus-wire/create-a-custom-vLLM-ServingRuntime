...
1. You are logged in to the OpenShift AI dashboard as a user with OpenShift AI administrator privileges (cluster-admin role in OpenShift).
1. The target image quay.io/vllm/automation-vllm:0.18.0rc1.dev7_rhaiv.0.ge974742fb.cpu is accessible from your cluster (it must be pullable; if in a disconnected environment, mirror it first).
1. You have the vLLM CPU ServingRuntime for KServe available (it is included in OpenShift AI 3.2 for CPU-only or IBM Z/Power scenarios).

## Step-by-step procedure (via OpenShift AI UI – recommended)

1. In the OpenShift AI dashboard, go to **Settings → Model resources and operations → Serving runtimes**.
  - This page lists all installed and enabled runtimes.
1. Locate the **built-in runtime named vLLM CPU ServingRuntime for KServe** (or the closest vLLM runtime matching your hardware; the .cpu tag indicates you want the CPU variant).
1. Click the **action menu (⋮)** next to it and select Duplicate.
...
In the embedded YAML editor, update the container image:YAML

```yaml
spec:
  containers:
  - name: kserve-container
    image: quay.io/vllm/automation-vllm:0.18.0rc1.dev7_rhaiv.0.ge974742fb.cpu   # ← Update this
    # ... other fields like resources, env, args remain the same
```
...
When you go to deploy a new model (Model Serving → Deploy model), it will appear in the Serving runtime dropdown. Select your custom one.
...
You can create the ServingRuntime directly with `oc apply -f custom-vllm-runtime.yaml` in the appropriate namespace (usually the project namespace or the OpenShift AI operator namespace). 
Start by:

1. listing the available images with `oc get servingruntime -n <namespace> | grep -i vllm`
1. exporting the built-in one with `oc get servingruntime vllm-cpu-servingruntime -o yaml` (adjust the name), modify the image field, and apply.


**IMPORTANT:** Red Hat does not provide support for custom runtimes (just that you can create customize the runtime) — test thoroughly.
